...
exports.generate = function(document) {
  ...
  // hash was specified, so we should always honor it (or force-generate a new one, if empty hash is supplied)
  if (document.hasOwnProperty('hash')) {
    if (document['hash'].length<1) {
...
exports.update = function(hash, updates) {
  ...
  ShortURL.findOne({hash: hash}, function(err, doc) {
    ...
    if (undefined != updates.hits) {
      doc.hits = updates.hits;
    }
    if (updates.data) {
...
exports.list = function(parms) {
  parms=!parms?{}:parms;
  return ShortURL.find(parms);
};
